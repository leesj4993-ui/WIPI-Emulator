...
class MainActivity : ComponentActivity() {
            ...
            when (screen) {
                Screen.Library -> LibraryScreen(
                    ...
                    onPlay = { entry ->
                        ...
                        } else errorMessage = pendingError() ?: getString(R.string.error_start_failed)
                    },
                    onImport = { uri ->
    ...
    private val hardwareKeyMap = mapOf(
        KeyEvent.KEYCODE_DPAD_UP to "UP", KeyEvent.KEYCODE_DPAD_DOWN to "DOWN",
        KeyEvent.KEYCODE_DPAD_LEFT to "LEFT", KeyEvent.KEYCODE_DPAD_RIGHT to "RIGHT",
        KeyEvent.KEYCODE_DPAD_CENTER to "OK", KeyEvent.KEYCODE_ENTER to "OK", KeyEvent.KEYCODE_SPACE to "OK",
        KeyEvent.KEYCODE_1 to "1", KeyEvent.KEYCODE_2 to "2", KeyEvent.KEYCODE_3 to "3",
        KeyEvent.KEYCODE_Q to "4", KeyEvent.KEYCODE_W to "5", KeyEvent.KEYCODE_E to "6",
        KeyEvent.KEYCODE_A to "7", KeyEvent.KEYCODE_S to "8", KeyEvent.KEYCODE_D to "9",
        KeyEvent.KEYCODE_Z to "*", KeyEvent.KEYCODE_X to "0", KeyEvent.KEYCODE_C to "#",
        KeyEvent.KEYCODE_4 to "4", KeyEvent.KEYCODE_5 to "5", KeyEvent.KEYCODE_6 to "6",
        KeyEvent.KEYCODE_7 to "7", KeyEvent.KEYCODE_8 to "8", KeyEvent.KEYCODE_9 to "9", KeyEvent.KEYCODE_0 to "0",
        KeyEvent.KEYCODE_STAR to "*", KeyEvent.KEYCODE_POUND to "#", KeyEvent.KEYCODE_DEL to "CLR",
        KeyEvent.KEYCODE_SHIFT_LEFT to "SOFT_L", KeyEvent.KEYCODE_SHIFT_RIGHT to "SOFT_R",
        KeyEvent.KEYCODE_F1 to "CALL", KeyEvent.KEYCODE_F2 to "HANGUP",
        KeyEvent.KEYCODE_BUTTON_A to "OK", KeyEvent.KEYCODE_BUTTON_B to "CLR",
        KeyEvent.KEYCODE_BUTTON_X to "*", KeyEvent.KEYCODE_BUTTON_Y to "#",
        KeyEvent.KEYCODE_BUTTON_L1 to "SOFT_L", KeyEvent.KEYCODE_BUTTON_R1 to "SOFT_R",
    )
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        ...
        if (event.repeatCount == 0) WipiNative.nativeKeyDown(key)
        return true
    }
    ...
    override fun onGenericMotionEvent(event: MotionEvent): Boolean {
        ...
        fun update(key: String, active: Boolean) {
            if (active && key !in stickPressed) {
                stickPressed += key; WipiNative.nativeKeyDown(key)
            } else if (!active && key in stickPressed) {
                stickPressed -= key; WipiNative.nativeKeyUp(key)
            }
        }
        update("LEFT", x < -0.5f); update("RIGHT", x > 0.5f)
        update("UP", y < -0.5f); update("DOWN", y > 0.5f)
        return true
    }
    ...
    private fun EmulatorScreen(onError: (String) -> Unit, onExit: () -> Unit, onSettings: () -> Unit) {
        ...
        BackHandler {
            if (paused) onExit() else { paused = true; WipiNative.nativeSetPaused(true) }
        }
        ...
        DisposableEffect(lifecycleOwner) {
            val observer = LifecycleEventObserver { _, event ->
                if (event == Lifecycle.Event.ON_PAUSE && !paused) {
                    paused = true; WipiNative.nativeSetPaused(true)
                }
            }
        ...
        LaunchedEffect(Unit) {
            while (isActive) {
                ...
                if (WipiNative.nativePollExit()) { onExit(); break }
                pendingError()?.let { onError(it); return@LaunchedEffect }
                delay(16)
            }
        }
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            Column(modifier = Modifier.fillMaxSize().background(Color(0xFF202020))) {
                Box(modifier = Modifier.fillMaxWidth().weight(1f), contentAlignment = Alignment.Center) {
                    frame?.let {
                        Image(bitmap = it, contentDescription = null, contentScale = ContentScale.Fit,
            ...
            Row(modifier = Modifier.align(Alignment.TopEnd)) {
                IconButton(onClick = {
                    if (!paused) { paused = true; WipiNative.nativeSetPaused(true) }
            ...
            if (paused) {
                Box(
                    modifier = Modifier.fillMaxSize().background(Color.Black.copy(alpha = 0.6f)).pointerInput(Unit) {
                        detectTapGestures { paused = false; WipiNative.nativeSetPaused(false) }
                    },
                    contentAlignment = Alignment.Center,
                ) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                        Icon(Icons.Filled.PauseCircle, null, tint = Color.White, modifier = Modifier.size(64.dp))
    ...
    private fun LibraryScreen(
        games: List<GameEntry>, error: String?, onDismissError: () -> Unit,
        onPlay: (GameEntry) -> Unit, onImport: (Uri) -> Boolean,
        onDelete: (GameEntry) -> Unit, onSettings: () -> Unit,
    ) {
        ...
        var saveTarget by remember { mutableStateOf<GameEntry?>(null) }
        var pendingExport by remember { mutableStateOf<GameEntry?>(null) }
        var pendingSaveImport by remember { mutableStateOf<GameEntry?>(null) }

        val saveExporter = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/zip")) { uri ->
            val target = pendingExport
            if (uri != null && target != null && !library.exportSave(target, uri))
                importError = "세이브 내보내기에 실패했습니다."
            pendingExport = null
        }
        val saveImporter = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
            val target = pendingSaveImport
            if (uri != null && target != null && !library.importSave(target, uri))
                importError = "세이브 불러오기에 실패했습니다."
                        ...
                        LazyVerticalGrid(columns = GridCells.Adaptive(minSize = 100.dp),
                            ...
                            items(games, key = { it.id }) { game ->
                                Column(horizontalAlignment = Alignment.CenterHorizontally,
                                    ...
                                    Text(game.name, fontSize = 12.sp, maxLines = 2, textAlign = TextAlign.Center,
                ...
                error?.let {
                    Text(it, color = MaterialTheme.colorScheme.onErrorContainer, fontSize = 13.sp,
                        ...
                        modifier = Modifier.align(Alignment.BottomCenter).fillMaxWidth()
                            ...
                            .pointerInput(Unit) { detectTapGestures { onDismissError() } }.padding(12.dp))
                }
            }
        }
        ...
        saveTarget?.let { target ->
            AlertDialog(
                ...
                text = { Text("세이브 파일을 내보내거나 수정한 세이브를 다시 불러올 수 있습니다.") },
                confirmButton = {
                    Column {
                        TextButton(onClick = {
                            pendingExport = target
                            saveTarget = null
                            saveExporter.launch("${target.name}_save.zip")
                        }) { Text("세이브 내보내기") }
                        TextButton(onClick = {
                            pendingSaveImport = target
                            saveTarget = null
                            saveImporter.launch(arrayOf("application/zip", "application/octet-stream"))
                        }) { Text("세이브 불러오기") }
                        TextButton(onClick = {
                            deleteTarget = target
                            saveTarget = null
                        }) { Text("게임 삭제", color = MaterialTheme.colorScheme.error) }
                    }
                },
                dismissButton = { TextButton(onClick = { saveTarget = null }) { Text("취소") } },
            )
        }
    ...
    Box(modifier = modifier.height(44.dp)
        ...
        .pointerInput(key) {
            detectTapGestures(onPress = {
                ...
                try { awaitRelease() } finally { pressed = false; WipiNative.nativeKeyUp(key) }
            })
        }, contentAlignment = Alignment.Center) {
